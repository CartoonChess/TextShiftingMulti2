...
// import { WarpTileScript } from './js/Tile.js'
// import Tile from './js/Tile.js'
import Tile, { WarpTileScript } from './js/Tile.js'
async function moveIfAble(character: Player, direction: Direction) {
    ...
    const tilesWithScripts: Tile[] = character.surroundings.thatInclude('scripts', Direction.Here)
    for (const tile of tilesWithScripts) {
        // Shouldn't happen, but skip in case scripts is undefined
        if (!tile.scripts) { break }
        
        for (const script of tile.scripts) {
            // const className = script.constructor?.name
            // if (className === 'WarpTileScript') {
            if (script instanceof WarpTileScript) {
...
game.inputController.move = (direction: Direction) => {
